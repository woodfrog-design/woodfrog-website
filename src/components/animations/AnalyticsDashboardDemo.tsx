...
const SpainMap: React.FC<{ highlightedRegion: string | null; onRegionHover?: (region: string | null) => void }> = ({ highlightedRegion, onRegionHover }) => {
    ...
    return (
        <svg viewBox="0 8 205 195" className="w-full h-full" style={{ filter: 'drop-shadow(0 2px 6px rgba(99,102,241,0.15))' }}>
            <defs>
                ...
                return (
                    <g key={city.name}>
                        ...
                        <circle
                            ...
                            fill={isRegionHighlighted ? '#4f46e5' : '#6366f1'}
        ...
        <div className="flex w-full h-full bg-[#fcfdfe] text-slate-800 overflow-hidden relative" style={{ fontFamily: 'var(--font-geist-sans)' }}>
            ...
            <aside className="w-[120px] bg-white/40 backdrop-blur-xl border-r border-white/40 flex flex-col shrink-0 relative z-10">
                ...
                <div className="px-4 pt-5 pb-4">
                    <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 via-indigo-600 to-violet-700 flex items-center justify-center shadow-lg shadow-indigo-200/50">
                            <Building2 size={16} className="text-white" />
                        </div>
                        <div className="hidden sm:block">
                            <span className="text-[11px] font-black tracking-tighter text-slate-900 block leading-none">METRIX</span>
                            <span className="text-[7px] text-indigo-500 font-bold uppercase tracking-wider">Analysis</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-3 mt-4">
                    ...
                    <nav className="space-y-1">
                        <div className="group flex items-center gap-2.5 px-3 py-2 bg-white shadow-sm border border-slate-100/50 text-indigo-600 rounded-xl cursor-not-allowed">
                ...
                <div className="px-3 py-4 mt-auto">
                    <div className="bg-white/50 border border-white/60 p-2 rounded-2xl flex items-center gap-2 shadow-sm">
                        <div className="w-6 h-6 rounded-lg bg-indigo-600 flex items-center justify-center shrink-0 shadow-md shadow-indigo-200">
                            <span className="text-[8px] text-white font-black">WF</span>
                        </div>
                        <div className="min-w-0">
                            <span className="text-[8px] text-slate-900 font-black block leading-none truncate underline decoration-indigo-200 decoration-2">Woodfrog</span>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <main className="flex-1 p-2.5 flex flex-col gap-2 overflow-hidden min-w-0">
                ...
                <div className="flex justify-between items-center relative z-10">
                    <div>
                        <h1 className="text-[13px] font-black text-slate-900 leading-none tracking-tight">Executive Overview</h1>
                        <p className="text-[8px] text-indigo-500/80 font-bold uppercase tracking-widest mt-1">Intelligence & Performance Core</p>
                    </div>
                    <div className="flex items-center gap-2" data-filter-bar>
                        {/* Year Toggle */}
                        <div className="flex bg-white/50 backdrop-blur-sm border border-white/60 rounded-xl p-1 gap-1 shadow-sm" data-year-filters>
                ...
                <div className="flex-1 grid grid-cols-12 grid-rows-[auto_1fr_1fr_auto] gap-2 min-h-0">
                    ...
                    <div className="col-span-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
                        <MetricCard title="Card Payment" amount={metrics.cardPayment.amount} percent={metrics.cardPayment.percent} change={metrics.cardPayment.change} isUp={metrics.cardPayment.isUp} icon={<CreditCard size={14} className="text-indigo-600" />} color="bg-indigo-50/50" glowColor="bg-indigo-500" />
                        ...
                        <MetricCard title="Fee" amount={metrics.fee.amount} percent={metrics.fee.percent} change={metrics.fee.change} isUp={metrics.fee.isUp} icon={<DollarSign size={14} className="text-violet-600" />} color="bg-violet-50/50" glowColor="bg-violet-500" />
                        <MetricCard title="Transfer" amount={metrics.transfer.amount} percent={metrics.transfer.percent} change={metrics.transfer.change} isUp={metrics.transfer.isUp} icon={<Repeat size={14} className="text-blue-600" />} color="bg-blue-50/50" glowColor="bg-blue-500" />
                                    ...
                                    <Bar dataKey="amount" radius={[6, 6, 6, 6]}>
                                        {barData.map((_, i) => (
                                            <Cell
                                                ...
                                                style={{ filter: i === barData.length - 1 ? 'drop-shadow(0 4px 6px rgba(79, 70, 229, 0.3))' : 'none' }}
                                            />
                                        ))}
                                    </Bar>
                                </BarChart>
                    ...
                    <div className="col-span-12 lg:col-span-7 bg-white/70 backdrop-blur-md rounded-3xl border border-white/40 p-5 flex flex-col shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex items-center gap-1 bg-white px-2 py-1 rounded-lg border border-slate-100 shadow-sm">
                                ...
                                <span className="text-[9px] text-slate-600 font-bold uppercase tracking-tight">Main Flow</span>
                            </div>
                        </div>
                        <div className="flex items-baseline gap-2 mb-1">
                        ...
                        <div className="flex-1 min-h-[80px] flex gap-5">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={lineData}>
                                        ...
                                        <Area
                                            ...
                                            activeDot={{ r: 5, fill: '#6366f1', stroke: '#fff', strokeWidth: 2, filter: 'drop-shadow(0 0 8px rgba(99, 102, 241, 0.5))' }}
                                        />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                            <div className="w-[110px] flex flex-col justify-center gap-3 border-l border-slate-100/50 pl-5">
                                {[
                                    { label: 'Late Fee', val: revenue.lateFee, color: 'text-indigo-600' },
                                    { label: 'Insurance', val: revenue.insurance, color: 'text-emerald-600' },
                                    { label: 'Credit Card', val: revenue.creditCard, color: 'text-violet-600' }
                                ].map((item, idx) => (
                                    <div key={idx}>
                    ...
                    <div className="col-span-12 lg:col-span-6 bg-white/70 backdrop-blur-md rounded-3xl border border-white/40 p-4 flex flex-col shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-all">
                        ...
                        <div className="flex-1 flex flex-col justify-center gap-2">
                            {products.map((p, i) => (
                                ...
                                >
                                    <span className="text-[8px] text-slate-500 font-bold w-[70px] truncate group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{p.name}</span>
                                    <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden relative">
                                        <motion.div
                                            ...
                                            style={{
                                                background: `linear-gradient(90deg, ${['#818cf8', '#6366f1', '#4f46e5', '#4338ca', '#3730a3'][i]}, ${['#a5b4fc', '#818cf8', '#6366f1', '#4f46e5', '#4338ca'][i]})`,
                                            }}
                                        >
                    ...
                    <div className="col-span-12 lg:col-span-6 bg-white/70 backdrop-blur-md rounded-3xl border border-white/40 p-4 flex flex-col shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white/80 transition-all overflow-hidden">
                        ...
                        <div className="flex gap-4 items-center flex-1">
                            ...
                            <div className="flex flex-col gap-1 flex-1">
                                {[
                                    { label: 'Online', value: recorded.online, color: 'bg-indigo-500', icon: <Monitor size={8} /> },
                                    { label: 'ATM', value: recorded.atm, color: 'bg-indigo-300', icon: <CreditCard size={8} /> },
                                    { label: 'Branch', value: recorded.branch, color: 'bg-indigo-100', icon: <Building2 size={8} /> },
                                ].map(item => (
                                    <div key={item.label} className="group flex items-center justify-between px-2 py-1.5 rounded-xl hover:bg-white/50 transition-all">
                                        <div className="flex items-center gap-2.5">
                                            <div className={`w-5 h-5 rounded-lg ${item.color} flex items-center justify-center text-slate-800 shadow-sm transition-transform group-hover:scale-110`}>
                                                {item.icon}
                                            </div>
