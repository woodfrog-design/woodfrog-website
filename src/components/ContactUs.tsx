...
// --- NEW: List of services for the dropdown ---
const services = [
...
// --- UPDATED: Added 'service' to FormData ---
interface FormData {
  firstName: string;
  lastName: string;
  email: string;
...
const ContactForm: React.FC<ContactFormProps> = ({
  ...
  const { isDarkTheme } = useTheme();
  // --- UPDATED: Added 'service' to initial state ---
  const [formData, setFormData] = useState<FormData>({
    firstName: '',
    lastName: '',
    email: '',
  ...
  const validateForm = (): boolean => {
    ...
    // Optional: Add validation for the service dropdown if needed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error submitting form:', error);
      // In a real app, you'd use a custom notification, not an alert.
        ...
        {submitSuccess ? (
          ...
          <form id="formdata" onSubmit={handleSubmit} className={styles.form}>
            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                ...
                <div className={styles.inputWrapper}>
                  ...
                  <input type="text" id="firstName" name="firstName" className={getInputStyle('firstName')} value={formData.firstName} onChange={handleChange} placeholder="Your first name" autoComplete="given-name" aria-required="true" pattern="[A-Za-z\s]+" title="Only letters are allowed" />
                </div>
                {formErrors.firstName && ( <div className={styles.errorText} role="alert"> {formErrors.firstName} </div> )}
              </div>
              <div className={styles.formGroup}>
                <label htmlFor="lastName" className={styles.label}> Last Name <span className={styles.required}>*</span> </label>
                <div className={styles.inputWrapper}>
                  <span className={styles.inputIcon}> <UserIcon /> </span>
                  <input type="text" id="lastName" name="lastName" className={getInputStyle('lastName')} value={formData.lastName} onChange={handleChange} placeholder="Your last name" autoComplete="family-name" aria-required="true" pattern="[A-Za-z\s]+" title="Only letters are allowed" />
                </div>
                {formErrors.lastName && ( <div className={styles.errorText} role="alert"> {formErrors.lastName} </div> )}
              </div>
            </div>
            <div className={styles.formGroup}>
              ...
              <div className={styles.inputWrapper}>
                ...
                <input type="email" id="email" name="email" className={getInputStyle('email')} value={formData.email} onChange={handleChange} placeholder="Your email address" autoComplete="email" aria-required="true" />
              </div>
              {formErrors.email && ( <div className={styles.errorText} role="alert"> {formErrors.email} </div> )}
            </div>
            <div className={styles.formGroup}>
              <label htmlFor="phone" className={styles.label}> Phone </label>
              <div className={styles.inputWrapper}>
                <span className={styles.inputIcon}> <PhoneIcon /> </span>
                <input type="tel" id="phone" name="phone" className={`${styles.input} ${isDarkTheme ? styles.darkInput : ''}`} value={formData.phone} onChange={handleChange} placeholder="+1-212-456-7890" autoComplete="tel" pattern="^\+[\d\s\-().]{7,20}$" inputMode="tel" maxLength={20} title="Enter a valid phone number starting with +. Allowed: digits, spaces, dashes, parentheses" />
              </div>
            </div>
            <div className={styles.formGroup}>
              ...
              <div className={styles.inputWrapper}>
                ...
                <input type="text" id="organization" name="organization" className={`${styles.input} ${isDarkTheme ? styles.darkInput : ''}`} value={formData.organization} onChange={handleChange} placeholder="Your company/organization (optional)" autoComplete="organization" />
              </div>
            </div>
            ...
            <div className={styles.formGroup}>
              ...
              <textarea id="message" name="message" className={`${styles.textarea} ${isDarkTheme ? styles.darkTextarea : ''} ${formErrors.message ? styles.inputError : ''}`} value={formData.message} onChange={handleChange} rows={4} placeholder="Type your message here" aria-required="true" ></textarea>
              {formErrors.message && ( <div className={styles.errorText} role="alert"> {formErrors.message} </div> )}
            </div>
            <div className={styles.formActions}>
              <CustomButton type="primary" htmlType="submit" disabled={isSubmitting} className={styles.submitButtonFull} >
                {isSubmitting ? (
                  <>
                    <svg className={styles.spinningIcon} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" > <path d="M21 12a9 9 0 1 1-6.219-8.56"></path> </svg>
                    Sending...
                  </>
                ) : (
                  <>
