...
const Items1: FunctionComponent<Items1Type> = ({
  ...
  return (
    <div className={[styles.items, className].join(' ')}>
      <img className={styles.subtractIcon} alt="" src={subtract} style={subtractIconStyle} />
      <img className={styles.indicatorsIcon} loading="lazy" alt="" src="/frame-2.svg" />
      <div className={styles.quotes} style={quotesStyle}>
        <div className={styles.items1}>
          <h3 className={styles.marcusSeptimus}>{marcusSeptimus}</h3>
          <div className={styles.dataScientistAt}>{dataScientistAtFintechTechnolog}</div>
        </div>
        <div className={styles.weStruggledTo}>{weStruggledToManageLargeVolum}</div>
      </div>
    </div>
  );
};
