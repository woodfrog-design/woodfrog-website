...
export const ProfitabilityDemo: React.FC = () => {
    ...
    const run = async () => {
      ...
      const getPos = (key: MetricKey) => {
        ...
        if (badge) {
          ...
          return {
            x: badgeRect.left - containerRect.left + badgeRect.width / 2,
            y: badgeRect.top - containerRect.top + badgeRect.height / 2,
          };
        }
        const btnRect = btn.getBoundingClientRect();
        return {
          x: btnRect.left - containerRect.left + 10,
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-white flex items-center justify-center overflow-hidden" style={{ fontFamily: 'var(--font-geist-sans)' }}>
      <div className="w-[400px] bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-[13px] font-semibold tracking-wide" style={{ color: ANIMATION_THEME.primary }}>Profitability</p>
            <p className="text-[11px] text-slate-400 font-medium">Financial Ratio</p>
          </div>
          <div className="w-9 h-9 rounded-full border-2 flex items-center justify-center" style={{ backgroundColor: `${ANIMATION_THEME.primary}1A`, borderColor: `${ANIMATION_THEME.primary}33` }}>
            <span className="text-[16px] font-bold" style={{ color: ANIMATION_THEME.primary }}>$</span>
          </div>
        </div>
            ...
            {legendKeys.map((key) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span
                    className={`text-[12px] font-semibold transition-colors duration-300 ${isActive ? 'text-slate-900' : 'text-slate-400'
                      }`}
                  >
                    {item.label}
                  </span>
