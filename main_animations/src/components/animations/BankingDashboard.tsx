...
const KpiCard: React.FC<{
...
}> = ({ title, value, trend, isUp, icon, sparkColor = ANIMATION_THEME.primary }) => (
    <div className="bg-white border border-gray-100/80 rounded-lg p-2 flex flex-col justify-between h-full">
        <div className="flex justify-between items-start">
            ...
            <div className="p-0.5 rounded flex items-center justify-center bg-white border border-gray-100 shadow-sm" style={{ color: ANIMATION_THEME.primary }}>{icon}</div>
        </div>
            ...
            <aside className="w-[140px] border-r border-gray-100/80 p-3 flex flex-col justify-between bg-white shrink-0">
                <div>
                    <div className="flex items-center gap-1.5 mb-5 px-0.5">
                        <div className="w-5 h-5 rounded-md flex items-center justify-center" style={{ backgroundColor: ANIMATION_THEME.primary }}>
                            <Layers size={9} style={{ color: ANIMATION_THEME.text.primary }} />
                        </div>
                        <span className="text-[11px] font-semibold tracking-tight" style={{ color: ANIMATION_THEME.text.primary }}>Woodfrog</span>
                    </div>

                    <nav className="space-y-0.5">
                        <p className="text-gray-400 text-[7px] uppercase font-medium tracking-wider px-2 mb-1.5">Platform</p>
                        <div className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer border" style={{ backgroundColor: ANIMATION_THEME.background, borderColor: ANIMATION_THEME.border, color: ANIMATION_THEME.text.primary }}>
                ...
                <div className="flex items-center gap-2 px-2 py-2 border-t mt-2" style={{ borderColor: ANIMATION_THEME.border }}>
                    <div className="w-5 h-5 rounded-full flex items-center justify-center text-[7px] font-semibold" style={{ backgroundColor: ANIMATION_THEME.primary, color: ANIMATION_THEME.text.primary }}>JL</div>
                    <div>
                        <p className="text-[9px] font-medium leading-none" style={{ color: ANIMATION_THEME.text.primary }}>James Liu</p>
            ...
            <main className="flex-1 p-3 flex flex-col gap-2 bg-[#fafbfc] overflow-hidden">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex bg-white border rounded-md p-0.5 gap-0.5" style={{ borderColor: ANIMATION_THEME.border }}>
                            <button className="px-2 py-0.5 rounded text-[8px] font-medium" style={{ backgroundColor: ANIMATION_THEME.primary, color: ANIMATION_THEME.text.primary }}>Live</button>
                ...
                <div className="grid grid-cols-6 gap-1.5">
                    ...
                    <KpiCard title="Token Rate" value="1.24M/s" trend="+340K/s WoW" isUp={true} icon={<Brain size={8} />} sparkColor={ANIMATION_THEME.primary} />
                    ...
                    <KpiCard title="Active Users" value="284K" trend="+12.4% WoW" isUp={true} icon={<Users size={8} />} sparkColor={ANIMATION_THEME.primary} />
                    <KpiCard title="Cost Savings" value="$482K" trend="-23% vs Q3" isUp={true} icon={<Zap size={8} />} sparkColor={ANIMATION_THEME.primary} />
                </div>
                                ...
                                <BarChart data={throughputData} barCategoryGap="20%">
                                    ...
                                    <Bar dataKey="value" radius={[2, 2, 0, 0]}>
                                        {throughputData.map((_, index) => (
                                            <Cell key={`cell-${index}`} fill={index >= 10 ? ANIMATION_THEME.primary : index >= 8 ? `${ANIMATION_THEME.primary}80` : '#e2e8f0'} />
                                        ))}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                                    ...
                                    <AreaChart data={requestVolume}>
                                        <defs>
                                            <linearGradient id="reqGrad" x1="0" y1="0" x2="0" y2="1">
                                                <stop offset="5%" stopColor={ANIMATION_THEME.primary} stopOpacity={0.12} />
                                                <stop offset="95%" stopColor={ANIMATION_THEME.primary} stopOpacity={0} />
                                            </linearGradient>
                                        </defs>
                                        <Area type="monotone" dataKey="v" stroke={ANIMATION_THEME.primary} strokeWidth={1.5} fill="url(#reqGrad)" dot={false} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                                ...
                                <tbody className="text-slate-600">
                                    {coreServices.map((s, i) => (
                                        <tr key={i} className="border-b border-gray-50 hover:bg-slate-50/50 cursor-pointer">
                                                ...
                                                <div className="flex items-center justify-end gap-1">
                                                    <div className="w-8 h-1 bg-slate-100 rounded-full overflow-hidden">
                                                        <div
                                                            ...
                                                            style={{ backgroundColor: s.load > 85 ? '#f59e0b' : ANIMATION_THEME.primary, width: `${s.load}%` }}
                                                        />
                                                    </div>
                                            ...
                                            <td className="py-1.5 text-right">
                                                <span className={`text-[6px] font-medium px-1 py-0.5 rounded-full ${s.status === 'Healthy' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                                                    }`}>{s.status}</span>
                                            </td>
                                        </tr>
                                    ))}
                ...
                <div className="grid grid-cols-12 gap-2 shrink-0">
                            ...
                            {[
                                ...
                                <div key={i}>
                                    ...
                                    <div className="h-0.5 w-full bg-slate-100 rounded-full overflow-hidden mt-0.5">
                                        <div className="h-full rounded-full" style={{ backgroundColor: ANIMATION_THEME.primary, width: `${fee.p}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="col-span-5 bg-white border border-gray-100/80 rounded-lg p-2.5">
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                            {[
                                { name: 'GPT-4o Tuned', score: '94.2%', delta: '+1.8%', color: ANIMATION_THEME.primary },
                                { name: 'Predict-v4', score: '88.7%', delta: '+3.2%', color: '#f59e0b' },
                                { name: 'Vision-X', score: '91.5%', delta: '+0.6%', color: ANIMATION_THEME.text.primary },
                                { name: 'Agent-Core', score: '87.3%', delta: '+2.1%', color: ANIMATION_THEME.accent.success },
                            ].map((m, i) => (
                    ...
                    <div className="col-span-4 bg-slate-900 rounded-lg p-2.5 text-white relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-20 h-20 blur-[50px] opacity-15 -mr-6 -mt-6" style={{ backgroundColor: ANIMATION_THEME.primary }} />
                        <div className="relative z-10">
                            <span className="text-[7px] font-medium uppercase tracking-wider" style={{ color: ANIMATION_THEME.primary }}>Monthly Savings</span>
                            <div className="flex items-end gap-1.5 mt-0.5">
                        ...
                        <div className="relative z-10 grid grid-cols-4 gap-1 mt-2 pt-2 border-t border-white/10">
                            {[
                                ...
                                <div key={i} className="flex flex-col items-center">
                                    ...
                                    <span className="text-[9px] font-semibold" style={{ color: ANIMATION_THEME.primary }}>{item.val}</span>
                                </div>
                            ))}
                        </div>
                    </div>
